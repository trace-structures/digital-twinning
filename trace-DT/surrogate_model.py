...
class SurrogateModel:
    ...
    async def plot_subtract_effects_and_alert(self, q, QoI, subtracted_params, threshold_ratio=0.1, xticks=True):
        alert = False
        remained_train_effects = await self.subtract_effects(self.X_train, pd.DataFrame(self.y_train), subtracted_params)
...
async def main():
    ...
    if generated_data:
        ...
        P1 = SimParameter('a', NormalDistribution(1.03,0.02))
        ...
        # Sampling
        x_sample = Q.sample(100, method='MC', random_seed=1997)
        x_sample = torch.from_numpy(x_sample)
        x_df = pd.DataFrame(x_sample, columns=Q.param_names())
        x_df.to_csv('monitoring_x_df.csv', index=False)
        ...
        y_df.to_csv('monitoring_y_df.csv', index=False)
        
        q = Q.sample(1, method='MC', random_seed=1997)
        q_df = pd.DataFrame(q, columns=Q.param_names())
        q_df.to_csv('monitoring_q_df.csv', index=False)
