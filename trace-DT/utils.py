...
def plot_shap_multiple_waterfalls(model, q, param_name=None, show_param_values=False):
    ...
    for pos, label in zip(group_positions, group_labels):
        ax.text(pos, y_min - y_range_ratio*1.6, label, ha='center', fontsize=11, fontweight='bold')
...
def plot_remained_effects(higher_bound, lower_bound, margin, higher_measured_bound, lower_measured_bound, QoI_names):
    ...
    for i in range(len(x)):
        if (higher_measured_bound[i] > higher_bound[i] + margin[i]) or (higher_measured_bound[i] < lower_bound[i] - margin[i]):
            ax.plot(i, higher_measured_bound[i], 'ro', label='Point outside accept region')

        if (lower_measured_bound[i] > higher_bound[i] + margin[i]) or (lower_measured_bound[i] < lower_bound[i] - margin[i]):
            ax.plot(i, lower_measured_bound[i], 'ro')
